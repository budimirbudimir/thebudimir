...
function AgentEditorModal({ opened, onClose, agent, onSave, availableModels }: AgentEditorModalProps) {
  ...
  return (
    ...
    >
      <Stack gap="md">
        ...
        <Box>
          <Text size="sm" fw={500} mb="xs">Model (optional)</Text>
          <SegmentedControl
            value={service}
            onChange={(value) => {
        ...
        <Box>
          <Text size="sm" fw={500} mb="xs">Temperature: {temperature.toFixed(1)}</Text>
          <Slider
            value={temperature}
        ...
        <Box>
          <Text size="sm" fw={500} mb="xs">Max Iterations: {maxIterations}</Text>
          <Slider
            value={maxIterations}
        ...
        <Box>
          <Text size="sm" fw={500} mb="xs">Tools</Text>
          <Switch
            label="Web Search"
...
function TeamEditorModal({ opened, onClose, team, onSave, agents }: TeamEditorModalProps) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  ...
  const toggleMember = (agentId: string) => {
    if (agentId === coordinatorAgentId) return; // Can't add coordinator as member
  ...
  return (
    ...
    >
      <Stack gap="md">
        ...
        <Box>
          <Text size="sm" fw={500} mb="xs">Coordinator Agent (required)</Text>
          <Text size="xs" c="dimmed" mb="xs">The coordinator receives tasks and delegates to team members</Text>
          <ScrollArea.Autosize mah={150}>
            <Stack gap="xs">
              {agents.map((agent) => (
                <Paper
                  ...
                  style={{
                    ...
                    backgroundColor: coordinatorAgentId === agent.id ? 'rgba(124, 58, 237, 0.1)' : undefined,
                  }}
                  onClick={() => {
                ...
                >
                  ...
                  {agent.description && <Text size="xs" c="dimmed">{agent.description}</Text>}
                </Paper>
              ))}
            </Stack>
          </ScrollArea.Autosize>
        ...
        <Box>
          <Text size="sm" fw={500} mb="xs">Team Members (specialists)</Text>
          <Text size="xs" c="dimmed" mb="xs">Select agents that the coordinator can delegate tasks to</Text>
          <ScrollArea.Autosize mah={150}>
            <Stack gap="xs">
              {agents.filter(a => a.id !== coordinatorAgentId).map((agent) => (
                <Paper
                  key={agent.id}
                  p="xs"
                  withBorder
                  style={{
                    ...
                    backgroundColor: memberAgentIds.includes(agent.id) ? 'rgba(16, 185, 129, 0.1)' : undefined,
                  }}
                ...
                >
                  <Group justify="space-between">
                    ...
                    {memberAgentIds.includes(agent.id) && (
                      <Badge size="xs" color="teal">Selected</Badge>
                    )}
                  </Group>
                </Paper>
              ))}
        ...
        <Box>
          <Text size="sm" fw={500} mb="xs">Execution Mode</Text>
          <SegmentedControl
            value={executionMode}
